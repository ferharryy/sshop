...
(function($) {
    $.fn.fullCalendar = function(options) {
        ...
        this.each(function() {
            ...
            var date = options.year ? // holds the year & month of current month
                        new Date(options.year, options.month || 0, 1) : new Date();
            ...
            if (bo) { // "button options"
                ...
                if (bo == true || bo.today !== false) {
                    todayButton = $("<button class='today' />").append($("<span />").html(typeof bo.today == 'string' ? bo.today : "today")).click(gotoToday);
                    buttons.append(todayButton);
                }
                if (bo.prevYear) {
                    var b = $("<button class='prev-year' />").append($("<span />").html(typeof bo.prevYear == 'string' ? bo.prevYear : "&laquo;")).click(prevYear);
                    if (r2l)
                        buttons.prepend(b);
                    else
                        buttons.append(b);
                ...
                if (bo == true || bo.prevMonth !== false) {
                    var b = $("<button class='prev-month' />").append($("<span />").html(typeof bo.prevMonth == 'string' ? bo.prevMonth : (r2l ? "&gt;" : "&lt;"))).click(prevMonth);
                    if (r2l)
                        buttons.prepend(b);
                    else
                        buttons.append(b);
                ...
                if (bo == true || bo.nextMonth !== false) {
                    var b = $("<button class='next-month' />").append($("<span />").html(typeof bo.nextMonth == 'string' ? bo.nextMonth : (r2l ? "&lt;" : "&gt;"))).click(nextMonth);
                    if (r2l)
                        buttons.prepend(b);
                    else
                        buttons.append(b);
                ...
                if (bo.nextYear) {
                    var b = $("<button class='next-year' />").append($("<span />").html(typeof bo.nextYear == 'string' ? bo.nextYear : "&raquo;")).click(nextYear);
                    if (r2l)
                        buttons.prepend(b);
                    else
                        buttons.append(b);
            ...
            monthElement = $("<div class='full-calendar-month' style='position:relative'/>").appendTo($("<div class='full-calendar-month-wrap'/>").appendTo(this));
            //
            ...
            function render() {
                ...
                if (!tbody) {
                    ...
                    for (var i = 0; i < 7; i++) {
                        ...
                        thead += "<th class='" + dayAbbrevs[j].toLowerCase() +
                        (i == 0 ? ' first' : '') +
                        "'>" +
                    ...
                    for (var i = 0; i < numWeeks; i++) {
                        ...
                        var tds = "";
                        for (var j = 0; j < 7; j++) {
                            var s = "<td class='day " + dayAbbrevs[(j + weekStart) % 7].toLowerCase() +
                            (j == dit ? ' first' : '') +
                    ...
                    glass = $("<div style='position:absolute;top:0;left:0;z-index:1;width:100%' />").appendTo(monthElement).click(function(ev, ui) {
                        if (options.dayClick) {
                            buildDayGrid();
                            var td = dayTD(ev.pageX, ev.pageY);
                            if (td)
                                return options.dayClick.call(td, dayDate(td));
                        }
                    });
                }
                else {
            ...
            function setCellSizes() {
                ...
                thead.find('th').filter(':lt(6)').width(cellw).end().filter(':eq(6)').width(tbodyw - cellw * 6);
                ...
                while (ws.getTime() < end.getTime()) {
                    ...
                    $.each(events, function(j, event) {
                        if (event.end.getTime() > ws.getTime() && event.start.getTime() < we.getTime()) {
                            ...
                            if (event.weekends === false) {
                                if (isStart && event.start.getDay() == 0)
                                    addDays(ss, 1, true); /* event starts on a Sunday, add 1 day */
                                if ((isEnd && !isStart) && event.end.getDay() == 0)
                                    return true; /* exclude segment */
                                if (isStart && event.start.getDay() == 6)
                                    return true; /* exclude segment */
                            }
                            segs.push({
                                event: event,
                            ...
                            if (r2l) {
                                if (seg.event.weekends === false) {
                                    left1 = seg.isEnd ?
                                            ...
                                            tr.find('td:eq(1) div.day-content div').position().left;
                                    left2 = seg.isStart ?
                                            tr.find('td:eq(' + ((seg.start.getDay() - weekStart + 7) % 7 * dis + dit) + ') div.day-content div') :
                                            tr.find('td:eq(5) div.day-content div');
                                }
                                else {
                                    left1 = seg.isEnd ?
                                            tr.find('td:eq(' + ((seg.end.getDay() + 6 - weekStart) % 7 * dis + dit) + ') div.day-content div').position().left :
                                ...
                                if (seg.event.weekends === false) {
                                    left1 = seg.isStart ?
											...
											tr.find('td:eq(1) div.day-content div').position().left;
                                    left2 = seg.isEnd ?
											tr.find('td:eq(' + ((seg.end.getDay() - weekStart + 6) % 7) + ') div.day-content div') :
											tr.find('td:eq(5) div.day-content div');
                                }
                                else {
                                    left1 = seg.isStart ?
            ...
            function buildEventText(event, element) {
                $("<span class='event-title' />").text(event.title).attr('title', buildEventTitle(event)).appendTo(element);
                ...
                if (st != false) {
                    ...
                    event.end.getMinutes())) {
                        ...
                        if (r2l)
                            element.append(timeElement.text(' ' + timeStr));
                        else
                            element.prepend(timeElement.text(timeStr + ' '));
                    }
                }
            }
            //
            // create the text-contents of an event title
            //
            function buildEventTitle(event) {
                ...
                if (event.weekends === false) {
                    if (start.getDay() == 0)
                        addDays(start, 1, false);
                    else if (start.getDay() == 6)
                        addDays(start, 2, false);
                ...
                return event.title + ' [' + prettyDate(start) + ' - ' + prettyDate(end) + ']';
            }
            //
            ...
            function draggableEvent(event, element) {
                element.draggable({
                    ...
                    start: function(ev, ui) {
                        ...
                        if (!dayOverlay)
                            dayOverlay = $("<div class='over-day' style='position:absolute;z-index:2' />").appendTo(monthElement);
                        buildDayGrid();
            ...
            function dayTD(x, y) {
                ...
                while (r < rmax && y > dayY0 + dayY[r + 1])
                    r++;
                while (c < cmax && x > dayX0 + dayX[c + 1])
                    c++;
                if (r < 0 || r >= rmax || c < 0 || c >= cmax)
                    return currTD = null;
                else
                    if (!currTD || r != currR || c != currC) {
                    currR = r;
                    currC = c;
    ...
    function normalizeEvent(event) {
        ...
        event.weekends = typeof event.weekends == 'undefined' ? true : typeof event.weekends == 'string' ? (event.weekends.toLowerCase() === 'true') : false;
        return event;
    }
    function segCmp(a, b) {
    ...
    $.fullCalendar = {
        ...
        dateFormatters: {
            ...
            'F': function(d) {
                return $.fullCalendar.monthNames[d.getMonth()]
            },
            'm': function(d) {
                return zeroPad(d.getMonth() + 1)
            },
            'M': function(d) {
                return $.fullCalendar.monthAbbrevs[d.getMonth()]
            },
            'n': function(d) {
                return d.getMonth() + 1
            },
